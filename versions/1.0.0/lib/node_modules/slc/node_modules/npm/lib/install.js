...
function install (args, cb_) {
  ...
  function cb (er, installed) {
    ...
    findPeerInvalid(where, function (er, problem) {
      ...
      if (unsupportedInstalled.length) {
        console.log(
          '\nSome of the packages installed are not supported by StrongLoop.' +
          '\nCheck the log messages above for more details.')
      }
...
function prettify (tree, installed) {
  ...
  return Object.keys(tree).map(function (p) {
    return archy({ label: visualiseSupport(tree[p].what) + " " + p
                 , nodes: (tree[p].children || []).map(function P (c) {
                     if (npm.config.get("long")) {
                       return {
                         label: visualiseSupport(c.what),
                         nodes: c.children.map(P)
                       }
                     }
                     var g = c.children.map(function (g) {
                       return visualiseSupport(g.what)
                     }).join(", ")
                     if (g) g = " (" + g + ")"
                     return visualiseSupport(c.what) + g
                   })
                 })
  }).join("\n")
}
...
function visualiseSupport(what) {
  var yellow = '\033[33m'
    , green = '\033[32m'
    , reset = '\033[0m';
  var supported = unsupportedInstalled.indexOf(what) === -1
    , color = supported ? green : yellow
  return color + what + reset;
}
...
function checkSupported(target, cb) {
  if (target.supported)
    return cb();
  
  // Enfore the block-unsupporte check
  if (npm.config.get('block-unsupported')) {
    var msg = format("%s@%s is not supported by StrongLoop.\n" +
                   "If you wish to install this package anyway, you can remove\n" +
                   "the `block-unsupported` option to bypass this check.",
                   target.name,
                   target.version);
    ...
    // Issue a warning
    log.warn("install", "%s@%s is not supported by StrongLoop.",
        // +"You can use --block-unsupported option to block the installation.", 
        target.name, 
        target.version)
    unsupportedInstalled.push(target.name + '@' + target.version);
    return cb();
  }
}
