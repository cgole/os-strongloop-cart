...
function addNameRange (name, range, data, cb) {
  ...
  function next () {
    ...
    // find the max *supported* satisfying version
    var supportedVersions = versions
      .filter(function(k) { return data.versions[k].supported })
    var ms = semver.maxSatisfying(supportedVersions, range, true)

    // find the max satisfying version.
    if (!ms) {
      ms = semver.maxSatisfying(versions, range, true)
    }
...
function addNameVersion (name, v, data, cb) {
  ...
  function next () {
    ...
    function fetchit () {
      var regConf = multireg.getConfig(npm.config, dist.tarball);
      if (!regConf.get('registry')) {
      ...
      tb.protocol = url.parse(regConf.get("registry")).protocol
      delete tb.href
      tb = url.format(tb)
