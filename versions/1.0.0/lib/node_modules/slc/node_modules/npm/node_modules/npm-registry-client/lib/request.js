...
function requestDone (method, where, cb) {
  return function (er, response, data) {
    ...
    var slRegistry = false;
    if(href.indexOf('strongloop.com') !== -1) {
      slRegistry = true;
    }
    if (er) {
      if(slRegistry) {
        er.message += ". Please contact support@strongloop.com."
      }
      return cb(er);
    }
    ...
    if (data && typeof data === "string" && response.statusCode !== 304) {
      // Catch the 5xx errors, assuming they are fatal
      if(response.statusCode >= 500) {
        var errMsg = response.statusCode + " "
          + require("http").STATUS_CODES[response.statusCode];

        // Check if the registry is hosted by strongloop.com
        if(slRegistry) {
          errMsg += ". Please contact support@strongloop.com.";
        }

        var err = new Error(errMsg)
        err.code = "E" + response.statusCode
        err.message = "\n" + data
        return cb(err, null, data, response)
      }
